    ...
    <div class="container">
      ...
      <div class="table">
        <el-table :data="tableData" stripe>
          <el-table-column label="管理员Id" prop="adminID" />
          <el-table-column label="管理员用户名" prop="adminName" />
          <el-table-column label="权限个数" prop="permission" />
          <el-table-column label="密码" prop="password" />
          <el-table-column align="center" label="操作" width="200px">
            <template slot-scope="scope">
              <el-button @click="edit(scope.row.adminID)" size="mini" type="success">编辑
              </el-button>
              <el-button @click="deleteItem(scope.row.adminID)" size="mini" type="danger">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-dialog :visible.sync="editing" title="编辑" width="30%">
        <el-form :model="entityForm" label-width="70px" ref="form">
          <el-form-item label="用户名">
            <el-input v-model="entityForm.adminName"></el-input>
          </el-form-item>
  ...
  methods: {
    ...
    addPermissionProp(data) {
      for (let i = 0; i < data.length; i++) {
        ...
        console.log("权限个数permisson=" + count);
      }
      return data;
    },
    create() {
